...
export const useClientConversionData = () => {
  ...
  const fetchClientConversionData = async () => {
    try {
      setLoading(true);
      ...
      const clientData: NewClientData[] = rows.slice(1).map((row: any[]) => ({
        memberId: row[0] || '',
        firstName: row[1] || '',
        lastName: row[2] || '',
        email: row[3] || '',
      ...
      console.log('Client conversion data loaded:', clientData.length, 'records');
      setData(clientData);
      setError(null);
    } catch (err) {
      console.error('Error fetching client conversion data:', err);
      setError('Failed to load client conversion data');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchClientConversionData();
  }, []);

  return { data, loading, error, refetch: fetchClientConversionData };
};
